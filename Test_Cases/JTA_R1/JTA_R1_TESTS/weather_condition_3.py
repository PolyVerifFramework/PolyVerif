...
sim.set_time_of_day(4.45, False)
...
state.transform.position = spawns[0].position + 30 * forward - 8 * right
...
pedState.transform.position = spawns[0].position + 55 * forward - 2 * right
ped = sim.add_agent("Bob", lgsvl.AgentType.PEDESTRIAN, pedState)
print(spawns[0].position + 75 * forward + 5 * right)
...
waypoints.append(lgsvl.WalkWaypoint(position=lgsvl.Vector(21.944195246164846, 1.7022884871983026, 11.711104828000328), idle=0))
