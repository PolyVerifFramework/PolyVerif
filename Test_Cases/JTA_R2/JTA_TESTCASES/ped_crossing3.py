...
state.transform.position = spawns[0].position +15* forward+ 2 * right
...
for i in range(1*10):
  ...
  start = spawns[0].position + (50 + (1.0 * (i//10))) * forward  + (0 + (2.0 * (i % 10))) * right
...
'''for i, name in enumerate(["SUV", "Jeep"],1):
    ...
    state.transform.position = spawns[0].position + (20 * forward) - (2.0 * i * right)
    state.transform.rotation = spawns[0].rotation
