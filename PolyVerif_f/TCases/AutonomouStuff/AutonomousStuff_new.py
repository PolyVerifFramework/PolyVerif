...
# )
t1 = sim.map_from_gps(
    northing=4137701.58401006,
    easting=596752.458051848,
    altitude=-19.421589770308,
    orientation=-228,  #225
)
...
for i, name in enumerate(["SUV"]):
    ...
    state1.transform.position = spawns[0].position + (30 * forward) - (5.0 * i * right) # + 10.0 * forward
...
#sim.run()
t0 = time.time()
sim.run(time_limit=28, time_scale=1)
t1 = time.time()
sim.stop()