...
statej = lgsvl.AgentState()
statej.transform.position = spawns[0].position + (30 * forward) - (5 * right)
statej.transform.rotation = spawns[0].rotation 
...
statej = lgsvl.AgentState()
statej.transform.position = spawns[0].position + (60 * forward) - (5 * right)
statej.transform.rotation = spawns[0].rotation 
statej.velocity = 25 * forward
suv1 = sim.add_agent("Jeep", lgsvl.AgentType.NPC, statej)
suv1.follow_closest_lane(True, 13.5)

right = lgsvl.utils.transform_to_right(spawns[0])

t0 = time.time()
sim.run(time_limit=18, time_scale=1)
t1 = time.time()
