...
egoState.transform.position = spawns[0].position + 25 * forward - 12 * right
egoState.velocity = 19 * forward
...
npcstate.transform.position = egoState.transform.position + 70.0 * forward - 19* right
npcstate.transform.rotation.y = spawns[0].rotation.y + 100
...
state.transform.rotation.y = spawns[0].rotation.y + 100
npc1 = sim.add_agent("SchoolBus", lgsvl.AgentType.NPC, state)
npc1.follow_closest_lane(True,9)
