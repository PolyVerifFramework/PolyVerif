...
class PathPlannerValidation(Node):
  ...
  def __init__(self):
    ...
    self.subGoalPos = self.create_subscription(PoseStamped,'/planning/goal_pose',self.goal_pose_callback,1)
  ...
  def goal_pose_callback(self, msg):
      ...
      with open(file_path + '/Goal_Pose_PathPlanning.csv','a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        pathplanner_GoalPose = PathPlanner_GoalPose(msg.header.frame_id, msg.header.stamp.sec, msg.header.stamp.nanosec, msg.pose.position.x, 
        msg.pose.position.y, msg.pose.position.z, msg.pose.orientation.x, msg.pose.orientation.y, msg.pose.orientation.z, 
