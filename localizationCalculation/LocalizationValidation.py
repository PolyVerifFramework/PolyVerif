...
def DeviationParams(location):
    ...
    localize_threshold_error= 5
    deviation_report = location + "/deviation_report.csv"
    deviation_data = Read_data(deviation_report) # Read per frame based deviation data
    
    maxDeviation = deviation_data.deviation.max()-localize_threshold_error # Calculate max deviation
...
def main(args=None):
   ...
   # print("Avp taltech  :", bool_val_taltech)
    ...
    auto_loc_file =   file_path + '/NDT_Pose_Localization.csv'
    lg_ego_file = file_path + '/GNSS_Localization.csv'
    
    auto_loc = Read_data(auto_loc_file)
    lg_ego_loc = Read_data(lg_ego_file)
    
    ComputeParams(auto_loc, lg_ego_loc,file_path, map_origion_error, avp_demo_flag)
    DeviationParams(file_path)
