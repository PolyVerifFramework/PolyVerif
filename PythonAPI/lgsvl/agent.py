...
class LidarContext:
    def __init__(self):
        self.NoisePercentage = 0.0  # [1..100 (+,-)]
        self.LidarDistance = 0.0  # [0..1]
...
class EgoVehicle(Vehicle):
    ...
    @accepts(GPSContext)
    def apply_gps_context(self,cntrl):
        args = {
            "uid": self.uid,
            "gpsData": {
                "Latitude": cntrl.Latitude,
                "Longitude" : cntrl.Longitude,
                "Altitude" : cntrl.Altitude,
                "Northing" : cntrl.Northing,
                "Easting" : cntrl.Easting,
            },
        }
        self.remote.command("sensor/apply_gps_context", args)

    @accepts(IMUContext)
    def apply_imu_context(self,cntrl):
        args = {
            "uid": self.uid,
            "ImuData": {
            "MeasurementSpan" : cntrl.MeasurementSpan,
            "Position" : cntrl.Position,
            "Orientation" : cntrl.Orientation,
            "Acceleration" : cntrl.Acceleration,
            "LinearVelocity" : cntrl.LinearVelocity,
            "AngularVelocity" : cntrl.AngularVelocity,
            "Percentage" : cntrl.Percentage,
            },
        }
        self.remote.command("sensor/apply_imu_context", args)

    @accepts(LidarContext)
    def apply_lidar_context(self,cntrl):
        args = {
            "uid": self.uid,
            "lidarNoise": {
                "NoisePercentage": cntrl.NoisePercentage,
                "LidarDistance": cntrl.LidarDistance,
            },
        }
        self.remote.command("sensor/apply_lidar_context", args)
