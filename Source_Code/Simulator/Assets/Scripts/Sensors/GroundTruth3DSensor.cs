...
namespace Simulator.Sensors
{
    ...
    public class GroundTruth3DSensor : SensorBase
    {
        ...
        void WhileInRange(Collider other)
        {
            ...
            //Debug.Log($"Game Object parent : {parent}");
            if (parent == egoGO)
            {
                return;
            }
            ...
            } else if (parent.layer == LayerMask.NameToLayer("BuildingLayer"))
            {
                //Changes_Rahul
                var pedC = parent.GetComponent<CustomControllerLayer>();
                id = pedC.GTID;
                label = "BuildingLayer";
                velocity = pedC.CurrentVelocity;
                angular_speed = -pedC.CurrentAngularVelocity.y;
                //Debug.Log("GT3D : BuildingLayer");

                //Rahul
                point_custom_layer.x = pedC.transform.position.x;
                point_custom_layer.y = pedC.transform.position.y;
                point_custom_layer.z = pedC.transform.position.z;
            }
            else
            {
                return;
