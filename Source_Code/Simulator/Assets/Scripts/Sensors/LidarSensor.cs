...
namespace Simulator.Sensors
{
    ...
    public partial class LidarSensor : LidarSensorBase
    {
        ...
        public static bool lidarNoisePercent = false;
        public static int varLidarNoisePercent = 0;

        public override void Reset()
        {
            lidarNoisePercent = false;          // reset the variable when the simulator is reset to false
            varLidarNoisePercent = 0;         // reset the variable when the simulator is reset to 0
        ...
        public void ApplyNoiseError(int ErrorNoise)
        {

            lidarNoisePercent = true;
            varLidarNoisePercent = ErrorNoise;
        }
            ...
            public void Execute()
            {
                ...
                for (int j = 0; j < LaserCount; j++)
                {
                    ...
                    for (int i = 0; i < Count; i++)
                    {
                        ...
                        int x = 0;
                        int y = 0;
                        int perX = ((int)(xx / XScale + TextureWidth / 2)) * (varLidarNoisePercent / 100);
                        int perY = ((int)(yy / YScale + TextureHeight / 2)) * (varLidarNoisePercent / 100);
                       
                        if(lidarNoisePercent){
                            x = ((int)(xx / XScale + TextureWidth / 2)) + perX;
                            y = ((int)(yy / YScale + TextureHeight / 2)) + perY;

                        }else{
                            x = (int)(xx / XScale + TextureWidth / 2);
                            y = (int)(yy / YScale + TextureHeight / 2);
                        }
        ...
        // Rahul API's added to expose to the user
        public NativeArray<Vector4> updatedPoints;
        public bool flag = false;
      
        public void ApplyContext(JSONArray Points_Data)
        {

            flag = true;
            int totalPointCount = LaserCount * MeasurementsPerRotation;
            int  i = 0;
            string jsonString = "";
            updatedPoints = new NativeArray<Vector4>(totalPointCount, Allocator.Persistent);
            int totalC = totalPointCount*4;
            for (int cnt = 0; cnt < totalC;)
            {
                Vector4 points;
                points.x = Points_Data[cnt++];
                points.y = Points_Data[cnt++];
                points.z = Points_Data[cnt++];
                points.w = Points_Data[cnt++];
        ...
        protected override void SendMessage()
        {
            ...
            // Send Data outside of the simulator using the API

            // Rahul Added the API custom call
            int totalPointCount = LaserCount * MeasurementsPerRotation;

            // var jsonData = new JSONObject();
            // // Working with all the value
            // float [] Arr = new float [totalPointCount*4];
            // int  i = 0;
            // string jsonString ="";
            // for (int cnt = 0; cnt < totalPointCount; cnt++) /* cnt < Points.length */
            // {
            //     Arr[i++] = Points[cnt].x;
            //     Arr[i++] = Points[cnt].y;
            //     Arr[i++] = Points[cnt].z;
            //     Arr[i++] = Points[cnt].w;
            // }
            // jsonString = JsonHelper.arrayToJson<float>(Arr);
            // jsonData.Add("Points",jsonString);

            //jsonData.Add("ObjectPosition",point_custom_layer);
            // Rahul TODO
            //Debug.Log($"Rahul Points : {point_custom_layer}");
            /////////////////////////////////////////////


            // if (ApiManager.Instance != null)
            // {
            //     ApiManager.Instance.AddCustom(transform.parent.gameObject, "mylidar", jsonData);
            // }

            // float percent =  (50/100);

            // NativeArray<Vector4> myPoints = new NativeArray<Vector4>(totalPointCount, Allocator.Persistent);
            // for (int cnt = 0; cnt < totalPointCount; cnt++)
            // {
            //     Vector4 points;
            //     points.x = Points[cnt].x + 20;// (Points[cnt].x * percent);
            //     points.y = Points[cnt].y;// (Points[cnt].y * percent);
            //     points.z = Points[cnt].z;// + (Points[cnt].z * percent);
            //     points.w = Points[cnt].w;
            //     myPoints[cnt] = points;

            // }

            // if(lidarNoisePercent){
            //     lidarNoisePercent = false;
            //     for (int p = 0; p < Points.Length; p++){
            //           Points[p] =  Points[p] * percent; // TODO
            ...
            //     updatedPoints.CopyTo(Points);
            //     if (updatedPoints.IsCreated)
            //     {
