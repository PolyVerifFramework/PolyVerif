...
gnome-terminal --tab --title="Scenario Run" --command="bash -c 'cd ../AutowareAuto; echo Trying to setup the scenario for running.. > ../Poly_Suite/logfiles/logInfo.txt; ade enter ./Poly_Suite/test_runner.sh; echo Simulation of selected Test cases completed, Click on Stop Scenario to check Report.. > ../Poly_Suite/logfiles/logInfo.txt; exit;$SHELL '"
