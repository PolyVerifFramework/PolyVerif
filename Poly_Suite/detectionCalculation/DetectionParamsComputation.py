...
class DetectionParams(object):
                ...
                for idx_Auto in range(auto_percep_data["timestamp_sec"].count()-1):
                    ...
                    if (gtd_data.timestamp_sec[idx_LG] == auto_percep_data.timestamp_sec[idx_Auto]):
                        if (gtd_stamping[idx_LG] == percep_stamping[idx_Auto]):
                            lg_pos = [gtd_data.position_x[idx_LG],
                                      gtd_data.position_y[idx_LG], 0]
                            
                            auto_pose_x_lst = [
                            auto_percep_data.corner_1_x[idx_Auto],
                            auto_percep_data.corner_2_x[idx_Auto],
                            auto_percep_data.corner_3_x[idx_Auto],
                            auto_percep_data.corner_4_x[idx_Auto]
                            ]   

                            auto_pose_y_lst = [
                            auto_percep_data.corner_1_y[idx_Auto],
                            auto_percep_data.corner_2_y[idx_Auto],
                            auto_percep_data.corner_3_y[idx_Auto],
                            auto_percep_data.corner_4_y[idx_Auto]
                            ]       


                            for value in auto_pose_x_lst:
                                nearest_val_x = None
                                min_difference = float('inf')
                                differene = abs(value - gtd_data.position_x[idx_LG])
                                if differene < min_difference:
                                    min_difference = differene
                                    nearest_val_x = value

                            for value in auto_pose_y_lst:
                                min_difference = float('inf')
                                nearest_val_y = None
                                differene = abs(value - gtd_data.position_y[idx_LG])
                                if differene < min_difference:
                                    min_difference = differene
                                    nearest_val_y = value

                            auto_pos = [nearest_val_x,
                                        nearest_val_y, 0]

                            # print("timestamp",auto_percep_data.timestamp_sec[idx_Auto])
                            # dist = distance.euclidean(lg_pos, auto_pos)
                            dist = math.sqrt(
                                (lg_pos[0]-auto_pos[0])**2 + (lg_pos[1]-auto_pos[1])**2 + (lg_pos[2]-auto_pos[2])**2)
                            diff_of_y = abs(auto_percep_data.centroid_y[idx_Auto] - lg_pos[1])

                            # print(lg_pos, ":", [auto_pos[0], auto_percep_data.centroid_y[idx_Auto]] )
    ...
    def ObjectDetection_Rate_MultiRange(matched_report, maxrange):
        ...
        multirange_detection_rate = pd.DataFrame(index=range(numRows), columns=[
                                                 'MinRange', 'MaxRange', 'SuccessRate', 'FailureRate'])
        count = 0
        temp = 0
        ...
        for idx_minrange in range(minrangeVal, maxrangeVal, interval):
            ...
            if (count_det_success != 0 or count_det_failure != 0):
                multirange_detection_rate.SuccessRate[count] = count_det_success*100 / (
                    count_det_success + count_det_failure)
                multirange_detection_rate.FailureRate[count] = count_det_failure*100 / (
                    count_det_success + count_det_failure)
            else:
